...
class EngagementService
{
    ...
    public function addComment(string $type, string $key, string $author, string $body, ?string $guestId = null): array
    {
        [$type, $key] = $this->resolve($type, $key);
    ...
    public function history(string $guestId, ?string $authorName = null): array
    {
        $comments = MediaComment::query()
            ->where(function ($q) use ($guestId, $authorName) {
                $q->where('guest_id', $guestId);
                if ($authorName) {
                    $q->orWhere('author_name', $authorName);
                }
            })
            ->latest()
            ->limit(20)
            ->get()
            ->map(fn (MediaComment $c) => $this->mapActivity('comment', $c->id, $c->target_type, $c->target_key, $c->body, $c->created_at?->toIso8601String()));

        $likes = MediaReaction::query()
            ->where('guest_id', $guestId)
            ->where('reaction', 'like')
            ->latest()
            ->limit(20)
            ->get()
            ->map(fn (MediaReaction $r) => $this->mapActivity('like', $r->id, $r->target_type, $r->target_key, null, $r->created_at?->toIso8601String()));

        $dislikes = MediaReaction::query()
            ->where('guest_id', $guestId)
            ->where('reaction', 'dislike')
            ->latest()
            ->limit(20)
            ->get()
            ->map(fn (MediaReaction $r) => $this->mapActivity('dislike', $r->id, $r->target_type, $r->target_key, null, $r->created_at?->toIso8601String()));

        return [
            'comments' => $comments->values()->all(),
            'likes' => $likes->values()->all(),
            'dislikes' => $dislikes->values()->all(),
        ];
    }

    private function mapActivity(string $kind, int $id, string $type, string $key, ?string $body, ?string $createdAt): array
    {
        $title = null;
        if ($type === 'video') {
            $title = Video::where('slug', $key)->value('title');
        } elseif ($type === 'song') {
            $title = Song::where('slug', $key)->value('title');
        } else {
            $title = 'Video YouTube';
        }
